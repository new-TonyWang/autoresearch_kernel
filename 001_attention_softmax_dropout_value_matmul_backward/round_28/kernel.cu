...
__global__ void __launch_bounds__(192, 6) attn_bw_v28(__nv_bfloat16* gs, float* gv,
    ...
    for (int d = tid; d < 128; d += 192) sg[d] = __bfloat162float(go[goff + d]);
    if (!tid) ss = 0; __syncthreads();
    int wp = 6, sp = (sv + wp - 1) / wp, ss2 = warp * sp, se = min(ss2 + sp, sv);
    float ls = 0;
    for (int s = ss2 + lane; s < se; s += 32) {
        float d = 0; #pragma unroll 16 for (int i = 0; i < 128; ++i) d += sg[i] * __bfloat162float(vs[voff + s * 128 + i]);
        bool m = dm[woff + s]; float g = m ? d * ds : 0; ls += g * __bfloat162float(aw[woff + s]);
        float a = __bfloat162float(ad[woff + s]); #pragma unroll 16 for (int i = 0; i < 128; ++i) atomicAdd(&gv[aoff + s * 128 + i], a * sg[i]);
    }
    ls = warp_sum(ls); __shared__ float spw[6]; if (!lane) spw[warp] = ls; __syncthreads();
    if (!warp) { ls = (lane < 6) ? spw[lane] : 0; ls = warp_sum(ls); if (!lane) ss = ls; }
    __syncthreads(); float ts = ss;
    for (int s = ss2 + lane; s < se; s += 32) {
        float d = 0; #pragma unroll 16 for (int i = 0; i < 128; ++i) d += sg[i] * __bfloat162float(vs[voff + s * 128 + i]);
        bool m = dm[woff + s]; float g = m ? d * ds : 0, a = __bfloat162float(aw[woff + s]);
        gs[woff + s] = __float2bfloat16(a * (g - ts));
    }
}

__global__ void cv(__nv_bfloat16* o, const float* i, int n) {
    int x = blockIdx.x * 256 + threadIdx.x; if (x < n) o[x] = __float2bfloat16(i[x]);
}
...
void attention_backward_launcher(torch::Tensor& gs, torch::Tensor& gv, const torch::Tensor& go,
    ...
    auto ga = torch::zeros({B, 8, sv, 128}, torch::TensorOptions().dtype(torch::kFloat32).device(gv.device()));
    attn_bw_v28<<<dim3(B, 80, sq), 192, 0, s>>>(
