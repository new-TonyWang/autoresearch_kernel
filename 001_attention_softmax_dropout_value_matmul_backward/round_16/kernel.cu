...
// Optimized for H200: Use shared memory for value staging
__global__ void __launch_bounds__(256, 4) attention_backward_v16(
    ...
    for (int sk = sk_start + lane; sk < sk_end; sk += 32) {
        ...
        for (int d = 0; d < HEAD_DIM; ++d) {
            dot += s_grad_out[d] * __bfloat162float(value_states[voff + d]);
        }
        ...
        // Accumulate grad_value in float buffer
...
// Conversion kernel
__global__ void __launch_bounds__(256, 4) convert_accum_to_bf16(
...
void attention_backward_launcher(
    ...
    // Create temp float accumulation buffer
    auto options = torch::TensorOptions().dtype(torch::kFloat32).device(grad_value_states.device());
    auto grad_value_accum = torch::zeros({batch_size, NUM_KV_HEADS, seq_len_kv, HEAD_DIM}, options);
    
    // Kernel 1: Compute grad_scores and accumulate grad_value
    dim3 grid1(batch_size, NUM_HEADS, seq_len_q);
    dim3 block(256);
    
    attention_backward_v16<<<grid1, block, 0, stream>>>(
    ...
    // Kernel 2: Convert float accumulation to bfloat16
    const int total_elements = grad_value_states.numel();
    dim3 grid2((total_elements + 255) / 256);
    
    convert_accum_to_bf16<<<grid2, block, 0, stream>>>(
