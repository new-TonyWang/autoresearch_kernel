...
std::vector<torch::Tensor> run(
    ...
    TORCH_CHECK(grad_attn_output.dtype() == torch::kBFloat16, "expected bfloat16");
    TORCH_CHECK(grad_attn_output.dim() == 4, "expected 4D tensor");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("run", &run, "Attention backward");
}
